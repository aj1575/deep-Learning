...
def get_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Per-modality drop probability during training (0-1).",
    )
    parser.add_argument(
        "--data-root",
        type=str,
...
def main():
    ...
    n_epochs = args.epochs if args.epochs is not None else (TEST_EPOCHS if TEST_MODE else TEACHER_EPOCHS)
    print(f"Training epochs: {n_epochs}")
    print(f"Training-time modality missing_prob: {args.missing_prob}")

    # Baseline-2 trains with random modality dropout.
    ...
    best_ckpt = os.path.join(ckpt_dir, "baseline2_best.pth")
    history_path = os.path.join(out_dir, "baseline2_history.json")
    ...
    for ep in range(1, n_epochs + 1):
        model.train()
        epoch_losses = []
        for imgs, segs, _ in tqdm(tr_loader, desc=f"Baseline2 {ep}/{n_epochs}", leave=False):
            imgs = imgs.to(device)
            segs = segs.to(device)
        ...
        history.append({"ep": ep, "loss": avg_loss, "missing_prob": args.missing_prob, **val})
        print(f"ep{ep:3d} | loss {avg_loss:.4f} | WT {val['WT']:.4f} TC {val['TC']:.4f} ET {val['ET']:.4f}")
