...
def main():
    ...
    ckpt = os.path.join(output_dir, "baselines", "unet_baseline", "checkpoints", "baseline2_best.pth")
    assert os.path.exists(ckpt), (
        f"Checkpoint not found: {ckpt}\n"
        "Train first: python baselines/unet_baseline/train_baseline2.py"
    )
    ...
    if "missing_prob" in state:
        print(f"Training missing_prob: {state['missing_prob']}")
    ...
    out_path = os.path.join(output_dir, "baselines", "unet_baseline", "baseline2_eval_results.json")
